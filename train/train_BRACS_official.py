...
def return_data_path_list():
    train_val_test_dict = joblib.load("../DataSplit/BRACS_official.pkl")
     
    train =  train_val_test_dict["train"]
    val =  train_val_test_dict["val"]
    test =  train_val_test_dict["test"]
...
def train_val_block(data_with_batch,model,optimizer,loss_fun,device,is_train_val_test,epoch_num,scheduler):
...
def main(args):
    ...
    handler = logging.FileHandler("../Logs/BRACS_official.log")
    handler.setLevel(logging.INFO)
    formatter = logging.Formatter('%(message)s')
    handler.setFormatter(formatter)
    logger.addHandler(handler)
    
    train_data_path_list,val_data_path_list,test_data_path_list = return_data_path_list()   
    train_data_with_batch = return_data_list_with_batch(train_data_path_list,BatchSize)
    val_data_with_batch = return_data_list_with_batch(val_data_path_list,BatchSize)
    ...
    for epoch_num in pbar:
        ...
        train_info_dict = train_val_block(train_data_with_batch,model,optimizer,loss_fun,device,"train",epoch_num,scheduler)
        print_info(epoch_num,Epoch,train_info_dict,"train",logger)

        val_info_dict = train_val_block(val_data_with_batch,model,optimizer,loss_fun,device,"val",epoch_num,scheduler)
        ...
        if val_info_dict["class_report"]["weighted avg"]["f1-score"] > best_f1:
            best_f1 = val_info_dict["class_report"]["weighted avg"]["f1-score"]
            torch.save(model.state_dict(), "../SavedModels/BRACS_official/best_val_f1_model.pt")

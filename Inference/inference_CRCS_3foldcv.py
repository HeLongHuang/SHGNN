...
class DSL(nn.Module):
    def __init__(self,
        ...
        self.x_tissue_2_attribute_layer = nn.Sequential(
            nn.Linear(feature_dim,feature_dim // 2),
            nn.LeakyReLU(),
            nn.Linear(feature_dim // 2,out_dim)
        )
        
        self.x_tissue_2_location_layer = nn.Sequential(
            nn.BatchNorm1d(tissue_centroid_dim),
            nn.Linear(tissue_centroid_dim,feature_dim // 8),
            nn.LeakyReLU(),
            nn.Linear(feature_dim // 8,out_dim)
    ...
    def forward(self,x_cell,centroids_cell,x_tissue_3,centroids_tissue_3,x_tissue_2,centroids_tissue_2):
        ...
        x_tissue_2_attribute = self.x_tissue_2_attribute_layer(x_tissue_2) 
        x_tissue_2_location = self.x_tissue_2_location_layer(centroids_tissue_2)
        ...
        x_tissue_2_attribute_loaction = torch.cat((x_tissue_2_attribute,x_tissue_2_location),dim = 1)
        ...
        batch = torch.zeros(x_tissue_2_attribute_loaction.shape[0],dtype = torch.long).to(device)
        tissue_2_edge = self.graph_edge(x = x_tissue_2_attribute_loaction,r = self.r,batch = batch,max_num_neighbors = self.n)
        
        return cell_edge,tissue_3_edge,tissue_2_edge
    
# cell + tissue3 + tissue2
class GCN(nn.Module):
    def __init__(self,
                Dropout = 0.25,
                Classes = 7,
        ...
        self.conv7 = SAGEConv(in_channels=FeatureDim,out_channels=ConvHiddenDim)          
        self.conv8 = SAGEConv(in_channels=ConvHiddenDim,out_channels=ConvHiddenDim)
        self.conv9 = SAGEConv(in_channels=ConvHiddenDim,out_channels=ConvOutDim)
        
        
        self.dsl = DSL(r = 5,n = 7,feature_dim = FeatureDim,cell_centroid_dim = 2,tissue_centroid_dim = 2,out_dim = LocationOutDim)
    ...
    def forward(self,data):
        ...
        x_cell,x_tissue_3,x_tissue_2,assignment_matrix_3,assignment_matrix_2,centroids_cell,centroids_tissue_3,centroids_tissue_2 = data.x_cell,data.x_tissue_3,data.x_tissue_2,data.assignment_matrix_3,data.assignment_matrix_2,data.centroids_cell,data.centroids_tissue_3,data.centroids_tissue_2
        
        # DSL
        edge_index_cell,edge_index_tissue_3,edge_index_tissue_2 = self.dsl(x_cell,centroids_cell,x_tissue_3,centroids_tissue_3,x_tissue_2,centroids_tissue_2)
...
#         batch = torch.where(assignment_matrix_3 == 1)[1].to(device)
        ...
        # tissue_2_path
        x_tissue_2_conv = x_tissue_2

        x_tissue_2_conv = self.conv7(x_tissue_2_conv,edge_index_tissue_2)
        x_tissue_2_conv = self.norm1(x_tissue_2_conv) 
        x_tissue_2_conv = self.relu(x_tissue_2_conv)  
        x_tissue_2_conv = self.dropout(x_tissue_2_conv)

        x_tissue_2_conv = self.conv8(x_tissue_2_conv,edge_index_tissue_2)
        x_tissue_2_conv = self.norm1(x_tissue_2_conv) 
        x_tissue_2_conv = self.relu(x_tissue_2_conv)  
        x_tissue_2_conv = self.dropout(x_tissue_2_conv)

        x_tissue_2_conv = self.conv9(x_tissue_2_conv,edge_index_tissue_2)
        x_tissue_2_conv = self.norm1(x_tissue_2_conv) 
        x_tissue_2_conv = self.relu(x_tissue_2_conv)  
        x_tissue_2_conv = self.dropout(x_tissue_2_conv)
        ...
        batch = torch.where(assignment_matrix_2 == 1)[1].to(device)
        x_tissue_2_for_cell = x_tissue_2_conv[batch].unsqueeze(1)
        x_cell_cov = x_cell_cov.unsqueeze(1)

        
        #Vision transformer
        x_cell_tissue = torch.cat((x_cell_cov,x_tissue_3_for_cell,x_tissue_2_for_cell),dim = 1)
...
#                 print("Inference for", pyg_data)
                pyg_data = joblib.load("../PYG_Data/CRCS/" + pyg_data)
...
def main(args):
    #load model path
    Model_path_1 = args.Fold1_Model_path
    Model_path_2 = args.Fold2_Model_path
    Model_path_3 = args.Fold3_Model_path
    
    
    #load test data
    BatchSize = args.BatchSize 
    CRCS_3foldcv = joblib.load("../DataSplit/CRCS_3foldcv.pkl")
    fold1_test_data_path_list = CRCS_3foldcv["fold1_test"]
    fold2_test_data_path_list = CRCS_3foldcv["fold2_test"]    
    fold3_test_data_path_list = CRCS_3foldcv["fold3_test"] 
    fold1_test_data_with_batch = return_data_list_with_batch(fold1_test_data_path_list,BatchSize)
    fold2_test_data_with_batch = return_data_list_with_batch(fold2_test_data_path_list,BatchSize)
    fold3_test_data_with_batch = return_data_list_with_batch(fold3_test_data_path_list,BatchSize)
    
    
    fold_num = 3
    for fold_num_temp in range(fold_num):
        print("****************  Fold{} Inference Process  ****************".format(fold_num_temp+1))
        #load model
        ...
        if fold_num_temp == 0:
            ...
            fold1_macro_auc, fold1_weighted_f1_score = test_block(fold1_test_data_with_batch,model,device)
        
        if fold_num_temp == 1:
            model.load_state_dict(torch.load(Model_path_2))
            #Inference and calculate metrics
            fold2_macro_auc, fold2_weighted_f1_score = test_block(fold2_test_data_with_batch,model,device)
            
        if fold_num_temp == 2:
            model.load_state_dict(torch.load(Model_path_3))
            #Inference and calculate metrics
            fold3_macro_auc, fold3_weighted_f1_score = test_block(fold3_test_data_with_batch,model,device)
        print("\n") 

    print("****************  Summary of 3-fold cv  ****************")   
    print("\n") 
    print("fold1_macro_auc = {}".format(fold1_macro_auc))
    print("fold2_macro_auc = {}".format(fold2_macro_auc))
    print("fold3_macro_auc = {}".format(fold3_macro_auc))
    print("Average_macro_auc = {}".format((fold1_macro_auc+fold2_macro_auc+fold3_macro_auc)/3))
    print("\n") 
    print("fold1_weighted_f1_score = {}".format(fold1_weighted_f1_score))
    print("fold2_weighted_f1_score = {}".format(fold2_weighted_f1_score))
    print("fold3_weighted_f1_score = {}".format(fold3_weighted_f1_score))
    print("Average_weighted_f1_score = {}".format((fold1_weighted_f1_score+fold2_weighted_f1_score+fold3_weighted_f1_score)/3)) 
...
def get_params():
    ...
    parser.add_argument("--Fold1_Model_path", type=str, default="../SavedModels/CRCS_3foldcv/fold1_best_val_auc_model.pt", help="")
    parser.add_argument("--Fold2_Model_path", type=str, default="../SavedModels/CRCS_3foldcv/fold2_best_val_auc_model.pt", help="")
    parser.add_argument("--Fold3_Model_path", type=str, default="../SavedModels/CRCS_3foldcv/fold3_best_val_auc_model.pt", help="")
    parser.add_argument("--BatchSize", type=int, default=30, help="")
    parser.add_argument("--Dropout", type=float, default=0.25, help="")
    parser.add_argument("--Classes", type=int, default=3, help="")
